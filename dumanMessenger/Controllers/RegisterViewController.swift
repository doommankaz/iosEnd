...
class RegisterViewController: UIViewController {
    ...
    @objc private func registerButtonTapped() {
        ...
        guard let firstName = firstNameField.text, let lastName = lastNameField.text, let email = emailField.text, let password = passwordField.text,
              !email.isEmpty, !password.isEmpty, !firstName.isEmpty, !lastName.isEmpty, password.count >= 8 else {
                  alertUserLoginError()
                  return
              }
        ...
        FirebaseAuth.Auth.auth().createUser(withEmail: email, password: password, completion: { authResult, error in
            ...
            print("User created: \(user)x")
        })
    }
    
    func alertUserLoginError() {
        let alert = UIAlertController(title: "Alert", message: "Please enter some information to create an account", preferredStyle: .alert)
